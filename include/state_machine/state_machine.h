...
public:
    ...
    StateMachine(): p_Owner(nullptr), p_CurrentState(nullptr), p_GlobalState(nullptr) {}
    
    StateMachine(entity_type* _owner, State<entity_type>* _state, State<entity_type>* _global): 
    ...
    entity_type* GlobalState() {
        return p_GlobalState; 
    }

    void Update() const {
        if(p_GlobalState) {
            p_GlobalState->Process();
        }

        if(p_CurrentState) {
            p_CurrentState->Process();
        }  
    }
