...
public:
    ...
    inline bool isZero() {
        return (x * x + y * y) < constants::MinDouble;
    }
...
//
const Vector &operator +(const Vector& rhs, const Vector& lhs){
    Vector vec = lhs;
    vec += rhs;
...
const Vector &operator *(double rhs, const Vector& lhs){
    Vector vec = lhs;
    vec *= rhs;
...
const Vector &operator *(const Vector& rhs, double lhs){
    Vector vec = rhs;
    vec *= lhs;
...
inline double VecLength(const Vector &v) {
    return sqrt(VecLengthSQ(v));
}

inline double VecLengthSQ(const Vector &v) {
...
Vector PointToLocalSpace(const Vector& point, 
...
Vector PointToWorldSpace(const Vector& localPoint,
                                const Vector& agentHeading,
                                const Vector& agentSide,
                                const Vector& agentPos) {
...
inline Vector Vector::Prep() const {
    return Vector(-y, x);
}

inline Vector Vector::GetReverse() const {
    return Vector(-x, -y);
}
