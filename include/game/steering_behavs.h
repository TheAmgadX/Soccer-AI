...
private:
  ...
  inline bool On(Behaviours b) const { return (m_Flags & b) == b; }
  ...
  bool DetectIntersection(const Vector &origin, const Vector &antenna,
                          const Vector &objFrom, const Vector &objTo,
                          Vector &hitPoint, double &distance);

  void FindIntersectionPlayers(SoccerTeam *team,
                               Vector &closest_intersect_point,
                               double &dist_to_closest_piont,
                               double detection_box_len, bool &found);

public:
  SteeringBehaviors(Player *_player, SoccerBall *_ball, SoccerPitch *_pitch)
      : p_Player(_player), p_Ball(_ball), p_Pitch(_pitch),
        m_SteeringForce(Vector()), m_Target(Vector()), m_InterposeDistance(0.0),
        m_Flags(none) {
    m_Antennas.resize(3);
  ...
  void PersuitOn() { m_Flags |= persuit; }

  void InterposeOn() { m_Flags |= interpose; }
  ...
  Vector Target() { return m_Target; }

  void SetTarget(const Vector &_target) { m_Target = _target; }

  Vector Force() { return m_SteeringForce; }

  bool IsSeekOn() { return On(seek); }

  bool IsArriveOn() { return On(arrive); }

  bool IsPersuitOn() { return On(persuit); }
