...
GoalKeeper::GoalKeeper(SoccerTeam *team, int home_region, Vector _heading)
    : Player(team, home_region, _heading) {
  p_StateMachine = new StateMachine<GoalKeeper>(
      this, GoalKeeperReturnHome::Instance(), GoalKeeperGlobal::Instance());
  p_StateMachine->CurrentState()->Enter(this);
}

GoalKeeper::~GoalKeeper() { delete p_StateMachine; }
...
Vector GoalKeeper::GetRearInterposeTarget() const {
  ...
  // Keep the keeper aligned with the ball along the goal mouth,
  // but constrained inside a defensive corridor near the posts.
  const double min_goal_y = constants::GOAL_TOP_Y + 20.0;
  const double max_goal_y = constants::GOAL_BOTTOM_Y - 20.0;
  const double y = std::clamp(Ball()->Pos().y, min_goal_y, max_goal_y);
